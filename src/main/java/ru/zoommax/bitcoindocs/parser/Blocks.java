...
public class Blocks {
    ...
    public Blocks(String raw) {
        ...
        for (int i = 0; i < lines.length; i++) {
            if (i < argumentsIndex || argumentsIndex == 0 && i< resultIndex) {
                parts[0] = parts[0] + lines[i]+ "\n";
            }
            if (i > argumentsIndex && i < resultIndex && argumentsIndex != 0) {
                parts[1] = parts[1] + lines[i]+ "\n";
            }
            if (i > resultIndex && i < exampleIndex  && resultIndex != 0) {
                parts[2] = parts[2] + lines[i]+ "\n";
            } else if (i > resultIndex && exampleIndex == 0 && resultIndex != 0) {
                parts[2] = parts[2] + lines[i]+ "\n";
            }
            if (i > exampleIndex  && exampleIndex != 0) {
